...
class Ktp extends R_Controller
{
  ...
  public function ambil_manual()
  {
    ...
    $this->load->page("ktp/ambil_antrian_manual")->layout("auth_layout");
  }

  public function fetch_form_manual()
  {
    if (!R_Input::isPost()) {
      show_404();
    }
    ...
    try {
      ...
      echo $this->load->component('pengunjung/form_pengunjung_manual', [
        'data' => $pengunjung ?? $data,
        'temp' => $temp
      ]);
    } catch (\Throwable $th) {
  ...
  public function simpan_manual()
  {
    ...
    try {
      ...
      if (!$perkara) {
        ...
        set_status_header(400);
        $this->load->component("table/table_sidang", $data);
        exit;
      }
      ...
      // $this->eloquent->connection("default")->transaction(function () use ($pengunjung) {});
    } catch (\Throwable $th) {
      Redirect::wfe($th->getMessage())->to("ktp");
    }
  }
