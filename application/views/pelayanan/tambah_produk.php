<div class="container-fluid py-3">
  <div class="d-flex justify-content-center">
    <div class="card col-md-6 col-lg-6 col-xl-6">
      ...
      <div class="card-body">
        <form autocomplete="off" class="form needs-validation" novalidate action="<?= base_url('pelayanan_produk/save') ?>" method="post" enctype="multipart/form-data">
          <input type="hidden" name="perkara_id" id="input-hidden-perkara-id">
          <input type="hidden" name="jenis_perkara" id="input-hidden-jenis-perkara">
          <div class="row">
            <div class="col-12">
              <div class="form-group mb-3">
                <label for="name">Nama Pengambil</label>
                <input type="text" required class="form-control" name="nama_pengambil" id="name">
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col-12">
              <div class="form-group">
                ...
                <div class="input-group mb-3">
                  <input class="form-control" required name="nomor_perkara" id="input-nomor-perkara" />
                </div>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="form-group">
              ...
              <div class="input-group mb-3">
                <input class="form-control" name="nomor_akta_cerai" id="input-nomor-akta-cerai" />
              </div>
            </div>
          </div>
          <div class="row">
...
<script>
  window.addEventListener("load", function() {
    ...
    const nomor_perkara_list = new Bloodhound({
      datumTokenizer: Bloodhound.tokenizers.whitespace,
      queryTokenizer: Bloodhound.tokenizers.whitespace,
      remote: {
        url: '<?= base_url('pelayanan_produk/nomor_perkara_autocomplete') ?>?query=%QUERY',
        wildcard: '%QUERY'
      }
    });

    $('#input-nomor-perkara').typeahead({
      hint: true,
      highlight: true,
      minLength: 1
    }, {
      name: 'nomor_perkara',
      source: nomor_perkara_list,
      display: 'nomor_perkara',
      limit: 9
    });

    $("#input-nomor-perkara").on("typeahead:select", function(e, s) {
      console.log(s)
      $("#input-nomor-akta-cerai").val(s.nomor_akta_cerai)
      $("#input-hidden-perkara-id").val(s.perkara_id)
      $("#input-hidden-jenis-perkara").val(s.jenis_perkara_text)
    })
  })
</script>